                ...
                <>
                    ...
                    >
                        <div className='overflow-x-auto'>
                            ...
                            <tbody>
                            {users.comments_user && Array.isArray(users.comments_user) &&
                              users.comments_user.map((item: any) => (
                                <tr key={item.id} onClick={() => router.push(`/comments/comments-view/?id=${item.id}`)}>
                        ...
                        {!users?.comments_user?.length && <div className={'text-center py-4'}>No data</div>}
                    </CardBox>
                </>

                <>
                    <p className={'block font-bold mb-2'}>Tracks Artist</p>
                    <CardBox
                      className='mb-6 border border-gray-300 rounded overflow-hidden'
                      hasTable
                    >
                        <div className='overflow-x-auto'>
                            ...
                            <tbody>
                            {users.tracks_artist && Array.isArray(users.tracks_artist) &&
                              users.tracks_artist.map((item: any) => (
                                <tr key={item.id} onClick={() => router.push(`/tracks/tracks-view/?id=${item.id}`)}>
                        ...
                        {!users?.tracks_artist?.length && <div className={'text-center py-4'}>No data</div>}
                    </CardBox>
                </>
